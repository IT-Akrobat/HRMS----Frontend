...
//   const [geoStatus, setGeoStatus] = useState("locating"); // locating | ok | denied | unavailable | unsupported
...
//                       />
//                       <span className="text-orange-600">
//                         Location access denied — enable it in your browser
//                         settings, then tap Detect Location.
//                       </span>
//                     </>
//                   )}
//                   {geoStatus === "unavailable" && (
...
//                       />
//                       <span className="text-orange-600">
//                         Couldn't get a location fix — this can happen indoors or
//                         with a weak signal. Tap Detect Location to try again.
//                       </span>
//                     </>
//                   )}
//                   {geoStatus === "unsupported" && (
...
// PERMISSION_DENIED means the browser (or the phone's OS) is refusing to
// even show the location prompt again -- "Detect Location" can't fix that
// on its own, since JS has no way to re-trigger a permission prompt once
// it's been blocked. The concrete fix is always a settings toggle, but
// *which* settings screen differs enough between iOS Safari, an
// installed/home-screen PWA, and Android Chrome that a single generic
// "enable it in your browser settings" line isn't actually actionable for
// most people. This picks the right short instruction for where the app
// is actually running right now.
function getLocationDeniedHelp() {
  const ua = navigator.userAgent || "";
  const isIOS = /iphone|ipad|ipod/i.test(ua);
  const isAndroid = /android/i.test(ua);
  // Standalone/home-screen PWA -- iOS exposes navigator.standalone;
  // everywhere else (incl. Android) the display-mode media query is the
  // reliable check. Matters because an installed PWA has its OWN location
  // permission, separate from the regular browser -- granting it in
  // Chrome/Safari does not carry over to the installed app icon.
  const isStandalone =
  ...
  if (isIOS && isStandalone) {
    return 'Open iPhone Settings → Privacy & Security → Location Services → find this app in the list → set to "While Using".';
  }
  if (isIOS) {
    return 'Open iPhone Settings → Privacy & Security → Location Services → Safari Websites → set to "Ask" or "While Using", then reload this page.';
  }
  if (isAndroid && isStandalone) {
    return "Open your phone's Settings → Apps → find this app → Permissions → Location → allow it, then reopen the app.";
  }
  if (isAndroid) {
    return "Tap the lock/site-info icon in the address bar → Permissions → Location → Allow, then reload this page.";
  }
  return "Click the lock/site-info icon next to the address bar → Site settings → Location → Allow, then reload this page.";
}
                ...
                <div className="flex items-start gap-2 min-w-0">
                  ...
                  {geoStatus === "denied" && (
                    <>
                      ...
                      />
                      <span className="text-orange-600">
                        Location access denied. {getLocationDeniedHelp()} A
                        blocked site won't re-show the popup on its own, so
                        tapping Detect Location again won't help until the
                        setting above is changed.
                      </span>
                    </>
                  )}
                  {geoStatus === "unavailable" && (
