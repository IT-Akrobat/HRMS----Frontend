...
/**
 * Working-Saturday is a separate, per-employee choice from weekday timing —
 * every area (Office, Inspection, Operation, Work Shop) has some staff who
 * work Saturdays and some who don't. sql/003_attendance_info_seed.sql
 * models this as a sibling shift row per area named "<AREA> - SATURDAY"
 * (e.g. "OFFICE - SATURDAY", "INSPECTION SITE - SATURDAY"), matching the
 * same "<area> - <label>" naming convention the backend's
 * _get_employee_shift() already parses. Given the employee's *weekday*
 * shift name, this finds that area's Saturday row so the UI can show its
 * hours read-only once HR flips "Works Saturdays?" to Yes.
 */
export function resolveSaturdayShift(shifts, weekdayShiftName) {
  if (!weekdayShiftName) return null;
  const area = weekdayShiftName.split(" - ")[0].trim().toUpperCase();
  return (
    shifts.find(
      (s) => (s.shift_name || "").trim().toUpperCase() === `${area} - SATURDAY`,
    ) || null
  );
}
